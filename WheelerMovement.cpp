...
void WheelerMovement::Move(char *filename)
{
	...
	while (counter == 0)
	{
		...
		vehicle = inst->createVehicle("18-WHEELER", filename);
		...
		if (vehicle->getxVehicleLocation() < xCenter)
		{
			if (turnDirection == LEFT)
			{
				...
				if (numLanes == 0)
				{
						...
						AC = vehicle->getSpeedMPS();
						BD = (numLanes == 4) ? (laneWidth * .5) : (laneWidth * .5);
						AD = xCenter - vehicle->getxVehicleLocation();
						BC = AC - AD + BD;
...
void WheelerMovement::PrintStatus()
{
